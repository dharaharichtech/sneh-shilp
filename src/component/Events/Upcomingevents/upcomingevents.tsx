...
import { MapPin, CalendarDays, HeartHandshake, ArrowDownCircle, ArrowLeft } from "lucide-react";
...
const UpcomingEventsSection: React.FC<UpcomingEventsProps> = ({
  ...
  return (
    <section className="bg-white w-full ">
      ...
      <div className="relative h-72 md:h-[420px] w-full overflow-hidden">
        ...
        {/* Banner Overlay Content (LEFT aligned) */}
        <div className="absolute inset-0 bg-black/40 flex flex-col justify-center pl-10 md:pl-20 text-white">
         <h4
             className={`${sueEllen.className} text-3xl text-white font-normal mb-5`}
            >
            Upcoming Events
        </h4>
           <h4
             className={`${calistoga.className} text-3xl text-white font-normal mb-5`}
            >
            Upcoming Events
        </h4>
          <button
            onClick={() => (window.location.href = "/blog")}
            className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-full text-sm md:text-base transition w-45 h-10"
          >
  ...
  <div className="bg-[#73BE5F] rounded-3xl flex flex-col md:flex-row items-center justify-between px-10 py-12 md:py-16 shadow-lg relative overflow-hidden">
    ...
    {/* 🤝 Right Image Section */}
     <div className="flex-1 flex justify-end items-end mt-10 md:mt-0 translate-x-10 translate-y-15">
                <Image
            src={volunteer.image}
